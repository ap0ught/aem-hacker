#!/usr/bin/env python3
"""
AEM Hacker - Security scanner for Adobe Experience Manager (AEM) web applications.

Performs dozens of checks against a target AEM URL to identify common
...
class Detector(BaseHTTPRequestHandler):
    """Minimal HTTP server that listens for inbound SSRF callback requests.
...
def http_request_multipart(
    ...
    """Send a multipart/form-data HTTP request and return the response.
...
def preflight(url, proxy=None, debug=False):
    """Return True if the target URL is reachable, False otherwise."""
    try:
...
def exposed_set_preferences(base_url, my_host, debug=False, proxy=None):
    """Check for exposed setPreferences.jsp which may allow reflected XSS via the keymap parameter."""
    r = random_string(3)
...
def exposed_merge_metadata(base_url, my_host, debug=False, proxy=None):
    """Check for exposed MergeMetadataServlet which may allow reflected XSS via the path parameter."""
    r = random_string(3)
...
def exposed_get_servlet(base_url, my_host, debug=False, proxy=None):
    """Check for an exposed DefaultGetServlet that may leak sensitive JCR node data."""
    r = random_string(3)
...
def exposed_querybuilder_servlet(base_url, my_host, debug=False, proxy=None):
    """Check for an exposed QueryBuilder JSON/Feed servlet that may leak sensitive content."""
    r = random_string(3)
...
def exposed_gql_servlet(base_url, my_host, debug=False, proxy=None):
    """Check for an exposed GQL servlet that may return sensitive JCR data."""
    r = random_string(3)
...
def exposed_guide_internal_submit_servlet_xxe(
    ...
    """Check for an exposed GuideInternalSubmitServlet vulnerable to XXE (CVE-2019-8086)."""
    r = random_string(3)
...
def exposed_post_servlet(base_url, my_host, debug=False, proxy=None):
    """Check for an exposed SlingPostServlet that may allow persistent XSS or RCE."""
    r = random_string(3)
...
def create_new_nodes(base_url, my_host, debug=False, proxy=None):
    """Check whether anonymous or default-credential users can create new JCR nodes via POSTServlet."""
...
def create_new_nodes2(base_url, my_host, debug=False, proxy=None):
    """Check whether Geometrixx sample users can create JCR nodes (indicates misconfigured sample content)."""
    CREDS = (
        "admin:admin",
        "author:author",
        "admin:password",
...
def exposed_felix_console(base_url, my_host, debug=False, proxy=None):
    """Check for an exposed Felix OSGi Web Console (admin:admin) which allows RCE via bundle upload."""
    r = random_string(3)
...
def exposed_wcmdebug_filter(base_url, my_host, debug=False, proxy=None):
    """Check for the WCMDebugFilter vulnerability (CVE-2016-7882) enabling reflected XSS."""
    r = random_string(3)
...
def exposed_wcmsuggestions_servlet(base_url, my_host, debug=False, proxy=None):
    """Check for an exposed WCMSuggestionsServlet that may allow reflected XSS."""
    r = random_string(3)
...
def exposed_crxde_crx(base_url, my_host, debug=False, proxy=None):
    """Check for exposed CRXDE Lite, CRX Explorer, CRX search/namespace pages, and Package Manager."""
    r = random_string(3)
...
def swf_xss(base_url, my_host, debug=False, proxy=None):
    """Check for AEM-bundled SWF files that may allow reflected XSS via Flash parameters."""
    SWFS = (
...
def deser_externaljob_servlet(base_url, my_host, debug=False, proxy=None):
    """Check for Java deserialization vulnerability in ExternalJobServlet (OOM payload probe)."""
    r = random_string(3)
...
def exposed_webdav(base_url, my_host, debug=False, proxy=None):
    """Check for exposed WebDAV access to the JCR (may enable CVE-2015-1833 or stored XSS)."""
    r = random_string(3)
...
def exposed_groovy_console(base_url, my_host, debug=False, proxy=None):
    """Check for an exposed Groovy Console that allows unauthenticated RCE."""
    r = random_string(3)
...
def exposed_acs_tools(base_url, my_host, debug=False, proxy=None):
    """Check for exposed ACS AEM Tools Fiddle endpoint that allows RCE."""
    r = random_string(3)
